...
describe('Identifier', () => {
  it('should create an instance with the provided value', () => {
    ...
    expect(id.getValue()).toEqual(idValue)
  })
  ...
  it('should return false when comparing with an identifier of a different class', () => {
    class OtherIdentifier<T extends Primitives | Primitives[]> extends Identifier<T> {
      constructor(value: T) {
        super(value)
      }
    }
    const idValue = '123' as const
    const id1 = new Identifier(idValue)
  ...
  it('should return false when comparing with an identifier of a different value', () => {
    const id1 = new Identifier<string>('123')
    const id2 = new Identifier<string>('456')
    expect(id1.equals(id2)).toBe(false)
  })
