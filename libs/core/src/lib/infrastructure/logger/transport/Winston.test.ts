...
jest.mock('winston', () => {
  ...
  const createLogger = jest.fn((options = {}) => {
    if (options.format?.formatName === 'invalidFormat') {
...
describe('WinstonTransport', () => {
  ...
  it('Log a message with level "info" using the WinstonTransport instance and verify that the message is logged correctly.', () => {
    ...
    const createLoggerMock = winston.createLogger as jest.MockedFunction<
      typeof winston.createLogger
    >
  ...
  it('Create WinstonTransport with a format that is not a valid winston format and check for error handling.', () => {
    const invalidFormat = { formatName: 'invalidFormat' } as unknown as winston.Logform.Format
    expect(() => new WinstonTransport({ format: invalidFormat })).toThrow(
